...
def generate_tgt_mask(sz, num_neg, device):
    mask = (torch.triu(torch.ones(sz, sz).to(device)) == 1).transpose(0, 1)
    tgt_mask = torch.zeros((sz * (1 + num_neg), sz * (1 + num_neg))).to(device)
    for i in range(0, sz * (1 + num_neg), sz):
        tgt_mask[i: i + sz, i: i + sz] = mask
    return tgt_mask
