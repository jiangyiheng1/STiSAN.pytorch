...
class LBSNDataset(Dataset):
    def __init__(self, filename):
        ...
        self.user_seq, self.user2idx, self.region2idx, self.n_user, self.n_region, self.region2poi = \
            self.filtering_cold_user(filename)
        self.region_stats()
    ...
    def split_build_matrix(self, max_len=100):
        ...
        for u in range(len(self)):
            ...
            print(u+1, "/", len(self) + 1, "Takes", T.time()-start_time_u, "s")
...
if __name__ == "__main__":
    ...
    train, test = dataset.split_build_matrix()
    joblib.dump(train, 'data/demo_train.data')
    joblib.dump(test, 'data/demo_test.data')